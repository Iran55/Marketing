...
if(isset($_POST['action']) && $_POST['action'] == 'senha'){
    ...
    $dataNasc = verificar_entrada($_POST['dataNasc']);
    ...
    /*   if ($linha['nomeUsuario'] !== $nomeUsuario) {
            ...
            $sql = $conecta->prepare("INSERT into usuario (nome, nomeUsuario, email, senha, telefone, dataNasc, dataCriacao) values(?, ?, ?, ?, ?, ?, ?)");
            $sql->bind_param("sssssss", $nomeCompleto, $nomeUsuario, $emailUsuario, $senha, $telefone, $dataNasc ,$dataCriacao);
            if ($sql->execute()) {
